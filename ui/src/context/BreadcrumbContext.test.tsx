...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Codis AI Board", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Codis AI Board",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Codis AI Board");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Codis AI Board");
    expect(buildDocumentTitle([], null)).toBe("Codis AI Board");
  });
});
